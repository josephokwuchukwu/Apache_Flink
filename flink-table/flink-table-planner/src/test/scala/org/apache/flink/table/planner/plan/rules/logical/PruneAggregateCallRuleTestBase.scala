...
import org.apache.flink.table.planner.utils.{BatchTableTestUtil, TableTestBase, TestDynamicTableSourceFactory}
...
abstract class PruneAggregateCallRuleTestBase extends TableTestBase {
  ...
  def setup(): Unit = {
    util.tableEnv.executeSql(s"""
                                ...
                                |  d1 BIGINT
                                |) WITH (
                                |  'connector' = '${TestDynamicTableSourceFactory.IDENTIFIER}',
                                |  'bounded' = 'true'
                                |)
                                |""".stripMargin)
